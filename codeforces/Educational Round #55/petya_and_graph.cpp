...
int main(){
  ...
  scanf("%d%d", &n, &m);
  PushRelabel pushRelabel(1 + m + n + 1);
  int s = 0, t = 1 + m + n;

  for(int i = 1; i <= n; ++i){
    int x;
    scanf("%d", &x);
    pushRelabel.AddEdge(m + i, t, x);
  }

  for(int x = 1; x <= m; ++x){
    int i, j, w;
    scanf("%d%d%d", &i, &j, &w);
    pushRelabel.AddEdge(s, x, w);
    pushRelabel.AddEdge(x, m + i, inf);
    pushRelabel.AddEdge(x, m + j, inf);
    ans += w;
  }

  ans -= pushRelabel.GetMaxFlow(s, t);
