...
bool IsReachable(const int n, const int m, const vector<pii>& byte_coordinates, const int k) {
...
int main() {
	...
	int ans = 0;
	for (int x = 1, y = byte_coordinates.size(); x <= y;) {
		int mid = (x + y) / 2;
		if (IsReachable(n, m, byte_coordinates, mid)) {
			ans = mid, x = mid + 1;
		} else {
			y = mid - 1;
		}
	...
	auto [fx, fy] = byte_coordinates[ans];
	printf("%d,%d\n", fx, fy);
