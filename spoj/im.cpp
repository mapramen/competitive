...
bool Solve() {
  ...
  PushRelabel push_relabel(1 + n + n + 1);

  for (int i = 1; i <= n; ++i) {
    push_relabel.AddEdge(i, n + i, 1 + (i == 2));
  }
  ...
  while (m--) {
    ...
    push_relabel.AddEdge(n + i, j, 1);
    push_relabel.AddEdge(n + j, i, 1);
  }

  push_relabel.AddEdge(source, 2, 2);
  push_relabel.AddEdge(n + 1, sink, 1);
  push_relabel.AddEdge(n + 3, sink, 1);

  return push_relabel.GetMaxFlow(source, sink) == 2;
}
