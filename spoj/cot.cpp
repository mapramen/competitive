...
int Query(int l, int r, Node* a, Node* b, Node* pa, Node* pb, int k){
  if(l == r){
    return l;
  }
  
  int mid = (0ll + l + r) / 2;
  int c = GetLeftChildCount(a) + GetLeftChildCount(b) - GetLeftChildCount(pa) - GetLeftChildCount(pb);

  return k <= c
    ? Query(l, mid, GetLeftChild(a), GetLeftChild(b), GetLeftChild(pa), GetLeftChild(pb), k)
    : Query(mid + 1, r, GetRightChild(a), GetRightChild(b), GetRightChild(pa), GetRightChild(pb), k - c);
}
...
void DFS(int i, int p){
  ...
  roots[i] = Insert(INT_MIN, INT_MAX, roots[p], a[i]);
...
int main(){
  ...
  while(q--){
    ...
    int v = LCA(i, j);
    printf("%d\n", Query(INT_MIN, INT_MAX, roots[i], roots[j], roots[v], roots[parent[v]], k));
  }
