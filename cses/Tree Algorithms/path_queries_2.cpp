...
int a[N], parent[N], d[N], subtreeSize[N], chainNumber[N], chainLeader[N],
    chainPosition[N], T[LOGN + 1][N], edgeToVertex[N];
Node* st[N];
...
void UpdateVertex(int i, int x) {
  st[chainNumber[i]]->Update(chainPosition[i], x);
}
...
int QueryToAncestor(int i, int k) {
  ...
  return max(ans,
             st[chainNumber[i]]->Query(chainPosition[k], chainPosition[i]));
}
