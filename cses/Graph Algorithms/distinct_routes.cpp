...
int main(){
  ...
  while(m--){
    int i, j;
    scanf("%d%d", &i, &j);
    pushRelabel.AddEdge(i - 1, j - 1, 1);
  }

  int ans = pushRelabel.GetMaxFlow(0, n - 1);
  ...
  for(int x = 1; x <= ans; ++x){
    ...
    for(int i = 0; i != n - 1; ){
      ...
      while(!pushRelabel.G[i].empty()){
        int j = pushRelabel.G[i].back().to, flow = pushRelabel.G[i].back().flow;
        pushRelabel.G[i].pop_back();
